...
export function EmojiDescription({
  ...
  const handleCopy = useCallback((emojiChar: string) => {
    copyEmoji(emojiChar);
    if (copyTimeoutRef.current) clearTimeout(copyTimeoutRef.current);
    setCopiedId(emojiChar);
            ...
            <div className='flex items-center gap-4'>
              <div className='group relative'>
                ...
                <button
                  ...
                  className='absolute inset-0 flex items-center justify-center gap-1.5 rounded-lg bg-[var(--color-action-default)]/80 opacity-0 group-hover:opacity-100 transition-opacity duration-150 focus:outline-none focus-visible:ring-2 focus-visible:ring-[var(--color-action-default)]'
                  aria-label={`Copy ${emoji.description}`}>
                  {copiedId === emoji.emoji ? (
                    <>
